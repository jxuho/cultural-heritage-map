...
const SidePanelItems = ({
  ...
  const userReview =
    reviews.find(
      (r) => typeof r.user !== 'string' && r.user?._id === currentUser?._id,
    ) || null;
  ...
  if (!selectedPlaceData)
    return <ErrorMessage message="Data retrieval failed." />;
      ...
      <main className="flex-1 overflow-y-auto custom-scrollbar bg-[#f9f9f9]">
        {isReviewsExpanded ? (
          <div className="bg-white min-h-full">
            ...
            {loadingReviews ? (
              <div className="p-10 text-center text-[10px] uppercase tracking-widest animate-pulse">
                Syncing Reviews...
              </div>
            ) : (
                ...
                <div className="grid gap-4 bg-white border border-black p-5 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                  {selectedPlaceData.address?.fullAddress && (
                    <div className="flex gap-4">
                      ...
                      />
                      <div>
                        <label className="block text-[9px] uppercase text-gray-400 font-bold tracking-tight mb-1">
                          Location
                        </label>
                  ...
                  {selectedPlaceData.openingHours && (
                    <div className="flex gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium leading-snug font-mono italic">
                          {selectedPlaceData.openingHours}
                        </p>
                      </div>
                    </div>
                    ...
                    {Object.entries(selectedPlaceData.originalTags).map(
                      ([key, value]) => {
                        ...
                        return (
                          <div
                            key={key}
                            className="text-[10px] flex overflow-hidden border border-gray-200"
                          >
                            <span className="px-2 py-1 bg-gray-100 text-gray-500 border-r border-gray-200">
                              {key}
                            </span>
                            {isLink ? (
                              <a
                              ...
                              <span className="px-2 py-1 bg-white text-gray-700">
                                {valString}
                              </span>
                            )}
                          </div>
