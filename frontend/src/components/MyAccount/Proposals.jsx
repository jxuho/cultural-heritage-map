...
import { useProposals, useProposalModeration } from "../../hooks/useCulturalSitesQueries";
...
const Proposals = () => {
  ...
  const renderProposedValue = (key, value, proposalType) => {
    if (key === 'location' && typeof value === 'object' && value !== null && value.type === 'Point' && Array.isArray(value.coordinates)) {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="mb-6 flex justify-end">
        <label htmlFor="sort-proposals" className="mr-2 text-gray-700 font-medium">Sort By:</label>
        <select
          id="sort-proposals"
          value={sortOption}
      ...
      {isModerationPending && <p className="text-blue-600 mt-2 text-center">Processing...</p>}
      {isModerationError && <p className="text-red-600 mt-2 text-center">Error: {moderationError.message}</p>}
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {sortedProposals.map((proposal) => (
          ...
          >
            <h3 className="text-xl font-semibold mb-2">{proposal.proposalType}</h3>
            ...
            {proposal.culturalSite && (
              <>
                <p className="text-gray-700 mb-1">
                  <strong>Cultural Site:</strong> {proposal.culturalSite.name || 'N/A'}
                </p>
                {proposal.culturalSite.description && (
                  <p className="text-gray-700 mb-1">
                    <strong>Description:</strong> {proposal.culturalSite.description}
                  </p>
                )}
                ...
                {proposal.culturalSite.website && (
                  <a href={proposal.culturalSite.website} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline mb-1 block">
                    <strong>Website</strong>
                  </a>
                )}
                {proposal.culturalSite.imageUrl && (
                  <img src={proposal.culturalSite.imageUrl} alt={proposal.culturalSite.name} className="w-full h-48 object-cover rounded-md mb-2" />
                )}
                {proposal.culturalSite.openingHours && (
                  <p className="text-gray-700 mb-1">
                    <strong>Opening Hours:</strong> {proposal.culturalSite.openingHours}
                  </p>
                )}
              </>
            )}
            ...
            <p className="text-gray-700 mb-1">
              <strong>Proposed By:</strong> {proposal.proposedBy?.email || 'N/A'}
            </p>
              ...
              <div className="mt-2">
                ...
                <ul className="list-disc list-inside text-sm text-gray-600">
                  {Object.entries(proposal.proposedChanges)
                    ...
                    .map(([key, value]) => (
                      <li key={key}>
                        {key}: {renderProposedValue(key, value, proposal.proposalType)}
                      </li>
                    ))}
                </ul>
              </div>
            ...
            {proposal.status === 'pending' && (
              <div className="mt-4 p-3 border rounded-md bg-gray-50">
                <label htmlFor={`adminNotes-${proposal._id}`} className="block text-sm font-medium text-gray-700 mb-1">
                  Admin Notes:
                </label>
                <textarea
                ...
                <div className="mt-3 flex gap-2">
                  <button
                    ...
                    disabled={isModerationPending || !adminNotes[proposal._id]?.trim()}
                  >
                    Accept
                  </button>
                  <button
