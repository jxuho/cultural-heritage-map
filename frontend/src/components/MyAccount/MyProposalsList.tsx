...
const MyProposalsList = () => {
  ...
  const renderProposedValue = (
    ...
    if (
      ...
      return (
        <code className="bg-zinc-100 px-1 rounded text-[10px]">{`COORD[${value.coordinates[0]}, ${value.coordinates[1]}]`}</code>
      );
    }
    ...
    if (proposalType === 'create') {
      ...
      const oldV =
        typeof value.oldValue === 'object' ? 'OBJ' : String(value.oldValue);
      const newV =
        typeof value.newValue === 'object' ? 'OBJ' : String(value.newValue);
      return (
  ...
  if (isError)
    return (
      <div className="max-w-2xl mx-auto mt-12 border-2 border-red-600 p-8 bg-red-50">
        <h3 className="text-red-600 font-black uppercase mb-2">
          System Error_
        </h3>
      ...
      {!sortedProposals.length ? (
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-2">
          {sortedProposals.map((proposal) => (
            <div
              key={proposal._id}
              className="border-2 border-black flex flex-col bg-white hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] transition-shadow"
            >
              {/* Card Header */}
              ...
              <div className="p-5 grow flex flex-col gap-4">
                {proposal.culturalSite && (
                  <div className="space-y-1">
                    ...
                    <p className="font-black uppercase text-base">
                      {proposal.culturalSite.name || 'Untitled'}
                    </p>
                  </div>
                )}
                  ...
                  <ul className="space-y-1.5">
                    {Object.entries(proposal.proposedChanges || {})
                      ...
                      .map(([key, value]) => (
                        ...
                        >
                          <span className="font-bold text-black min-w-20">
                            {key}:
                          </span>
                ...
                {proposal.adminComment && (
                  <div className="mt-2 p-3 border-2 border-black bg-zinc-900 text-white">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] mb-1 text-zinc-400 italic underline">
                      Admin Verdict_
                    </p>
